    ...
    return (
        <main className="body-page">
            ...
            <section className="body-header">
                ...
                <p className="body-count">{products.length} {t('serviceCount')}</p>
            </section>

            {/* PDF disclaimer */}
            <div className="body-info-banner">
                ...
                <div onClick={() => setSelectedProduct(null)} className="body-modal-backdrop">
                    <div onClick={e => e.stopPropagation()} className="body-modal-panel">
                        ...
                        <p className="body-modal-meta">
                            {selectedProduct.type === 'pakiet' ? t('type.package') : selectedProduct.type === 'ebook' ? t('type.pdf') : t('type.session')} · {selectedProduct.duration}
                        </p>
                        {selectedProduct.availability && (
                            <div className="body-modal-section">
                                <p className="body-modal-label">📍 {t('modal.availability')}</p>
                                <p className="body-modal-text">{selectedProduct.availability}</p>
                            </div>
                        )}
                        {selectedProduct.includes && selectedProduct.includes.length > 0 && (
                            <div className="body-modal-section">
                                <p className="body-modal-label">{t('modal.includes')}</p>
                                <ul className="body-modal-list">
                                    {selectedProduct.includes.map((item, i) => <li key={i}>{item}</li>)}
                                </ul>
                            </div>
                        )}
                        {selectedProduct.whoFor && selectedProduct.whoFor.length > 0 && (
                            <div className="body-modal-section">
                                <p className="body-modal-label">{t('modal.whoFor')}</p>
                                <ul className="body-modal-list">
                                    {selectedProduct.whoFor.map((item, i) => <li key={i}>{item}</li>)}
                                </ul>
                            </div>
                        )}
                        {selectedProduct.warning && (
                            <div className="body-warning-box">
                                <p className="body-modal-label">⚠️ {t('modal.warning')}</p>
                        ...
                        <div className="body-modal-actions">
                            ...
                            >
                                {t('button.addToCartFull')} · {formatPrice(selectedProduct.priceGBP)}
                            </button>
                            <button type="button" onClick={() => setSelectedProduct(null)} className="body-modal-secondary-button">
                                {t('button.close')}
                            </button>
                        </div>
                    </div>
